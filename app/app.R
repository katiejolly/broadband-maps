...
plot_styling <- list(
  ...
  theme(
    ...
    plot.caption = element_text(margin = margin(0, 0, 30, 0), size = caption_size, lineheight = 0.8, family = "Lato", face = "italic"),
    plot.subtitle = element_text(family = "Karla", size = subtitle_size),
    plot.margin = unit(c(1, 1, 1, 1), "cm")
  ),
  annotation_scale(plot_unit = "m", line_width = 0.8, bar_cols = c("gray40", "white"), text_family = "Lato", location = "br", style = "ticks", text_cex = 1.4, unit_category = "imperial")
)
  ...
  plot_start_county <- eventReactive(input$goCounty, {
    if (input$background_county == TRUE) {
      p_county <- ggplot(county()) +
        ...
        geom_sf(fill = NA, color = "gray20", lwd = 0.3, linetype = 2, show.legend = FALSE)
    } else {
      p_county <- ggplot(county()) +
        # fancy_legend +
        geom_sf(fill = "gray99", color = "gray20", lwd = 0.1, show.legend = FALSE)
    }
  })
  ...
  output$county_map <- renderPlot({
    withProgress(message = "Drawing map...", {
      p_county <- plot_start_county() +
        geom_sf(data = tiles_county(), aes(fill = avg_d_mbps_cat), color = "gray15", lwd = 0.04, alpha = 0.8) +
        geom_text_repel(
          data = label_cities_county(),
      ...
      vline_color_county <- case_when(
        county_weighted_mean < 10 ~ map_colors[1],
        county_weighted_mean < 25 ~ map_colors[2],
        county_weighted_mean < 100 ~ map_colors[3],
        county_weighted_mean >= 100 ~ map_colors[4]
      )

      dist_subtitle_county <- glue("Weighted Mean:\n{county_weighted_mean} Mbps")
